                    ...
                    <div class="align-items-center">
                        <div class="" style="margin-top: 2em">
                            ...
                            <table class="table table-bordered table-hover text-center">
                                ...
                                if (!isset($_GET['pesquisar'])) {
                                    echo "<script>location.href ='../pesquisa_reparo.php'</script>";
                                } else {
                                    $valor_pesquisar = $_GET['pesquisar'];
                                }
                                $consulta = $conexao->query("SELECT * FROM `reparo` INNER JOIN statu ON reparo.status_id_status = statu.id_status INNER JOIN tecnico  ON reparo.tecnico_id_tecnico = tecnico.id_tecnico 
                                        INNER JOIN eletronico ON reparo.eletronico_id_eletronico = eletronico.id_eletronico INNER JOIN endereco ON tecnico.endereco_id_endereco = endereco.id_endereco 
                                        INNER JOIN cliente_eletronico ON cliente_eletronico.eletronico_id_eletronico = eletronico.id_eletronico INNER JOIN cliente ON cliente.id_cliente = cliente_eletronico.cliente_id_cliente 
                                        INNER JOIN login ON cliente.login_id_login = login.id_login WHERE cliente.login_id_login = $id_session AND modelo LIKE '%$valor_pesquisar%' OR data_entrega LIKE '%$valor_pesquisar%'");
                                ...
                                ?>
                                        <tr class="text-center">
                                            ...
                                            <td><a href="../../editar/editar_aparelho_reparo.php?id=<?= ($id) ?>" type="submit"><i class="bi bi-pencil-square"></i> Editar</a> </td>
                                        </tr>
                                    <?php
                                    }
                                    ?>
                        ...
                        <?php
                                    $select = $conexao->query("SELECT COUNT(id_reparo) AS num_result FROM `reparo` INNER JOIN statu ON reparo.status_id_status = statu.id_status 
                                        ...
                                        INNER JOIN cliente ON cliente.id_cliente = cliente_eletronico.cliente_id_cliente INNER JOIN login ON cliente.login_id_login = login.id_login 
                                        WHERE cliente.login_id_login = $id_session  AND modelo LIKE '%$valor_pesquisar%' AND data_entrega LIKE '%$valor_pesquisar%'");
                                    $row_pg = $select->fetch();
