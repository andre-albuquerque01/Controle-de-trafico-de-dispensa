            ...
            <div class="content-body">
                <div class="container">
                    ...
                    <div class="align-items-center">
                        <div class="">
                            ...
                            <table class="table table-bordered table-hover text-center">
                                ...
                                if (!isset($_GET['pesquisar'])) {
                                    echo "<script>location.href ='../pesquisa_cliente.php'</script>";
                                    //header("Location: ../pesquisa_cliente.php");
                                } else {
                                    $valor_pesquisar = $_GET['pesquisar'];
                                }
                                $consulta = $conexao->query("SELECT * FROM cliente INNER JOIN endereco ON endereco_id_endereco = id_endereco INNER JOIN login ON cliente.login_id_login = login.id_login WHERE login_id_login = $id_session AND cliente.nome_completo LIKE '%$valor_pesquisar%'");
                                
                                if ($consulta && $consulta->rowCount() != 0) {
                                    while ($sql = $consulta->fetch(PDO::FETCH_ASSOC)) {
                                        $nome = $sql['nome_completo'];
                                        $telefone = $sql['telefone_celular'];
                                        $cep = $sql['cep'];
                                        $cidade = $sql['cidade'];
                                        $uf = $sql['uf'];
                                        $bairro = $sql['bairro'];
                        ...
                        <?php
                                    $select = $conexao->query("SELECT COUNT(id_cliente) AS num_result FROM cliente INNER JOIN endereco ON endereco_id_endereco = id_endereco INNER JOIN login ON cliente.login_id_login = login.id_login WHERE login_id_login = $id_session AND cliente.nome_completo LIKE '%$valor_pesquisar%'");
                                    $row_pg = $select->fetch();
