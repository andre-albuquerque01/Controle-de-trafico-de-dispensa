                        ...
                        <div class="">
                            ...
                            <table class="table table-bordered table-hover text-center">
                                ...
                                if (!isset($_GET['pesquisar'])) {
                                    echo "<script>location.href ='../pesquisa_usuario.php'</script>";
                                    //header("Location: ../pesquisa_cliente.php");
                                } else {
                                    $valor_pesquisar = $_GET['pesquisar'];
                                }

                                $consulta = $conexao->query("SELECT * FROM login WHERE nome_completo_login LIKE '%$valor_pesquisar%'");
                                if ($consulta && $consulta->rowCount() != 0) {
                                    while ($sql = $consulta->fetch(PDO::FETCH_ASSOC)) {
                                        $nome_completo_login = $sql['nome_completo_login'];
                                        $nome_loja = $sql['nome_loja'];
                                        $email_usuario = $sql['email_usuario'];
                                        $id = $sql['id_login'];
                                ...
                                ?>
                                        <tr class="text-center">
                                            ...
                                            <td><a href="../../../model/editar/editar_usuario.php?id=<?= $id ?>" type="submit"><i class="bi bi-pencil-square"></i>
                                                    Editar</a>
                                    ...
                                    ?>
                                <?php
                                    $select = $conexao->query("SELECT COUNT(id_login) AS num_result FROM  login  WHERE nome_completo_login LIKE '%$valor_pesquisar%' ORDER BY nome_completo_login ASC");
                                    $row_pg = $select->fetch();
